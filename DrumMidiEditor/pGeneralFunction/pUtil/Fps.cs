...
public class Fps
{
	...
	/// FPS制御ディクショナリ
	/// </summary>
	private readonly Dictionary<int, TimeCount> _TimeCountDic = new();
	...
	public double NowTotalTime 
		=> (double)_StopWatch.ElapsedTicks / (double)Stopwatch.Frequency;
	...
	public double GetFrameTime( int aKey )
    {
		if ( !_TimeCountDic.TryGetValue( aKey, out var item ) )
        {
			return 0;
