...
public class NAudioData : DisposeBaseClass
{
    ...
    /// <returns>周波数別Dbリスト</returns>
    public List<float> GetFFTBuffer( Rectangle aRange, float aDbFileter )
    {
        var list = new List<float>();

        if ( _FFTBuffer != null )
        { 
            for ( int y = aRange.Top; y < aRange.Bottom; y++ )
            { 
                for ( int x = aRange.Left; x < aRange.Right; x++ )
                {
    ...
    /// </summary>
    /// <returns>True:読込完了、False:読込未完了</returns>
    public bool IsEnableFFT() => _FFTLoad; 
