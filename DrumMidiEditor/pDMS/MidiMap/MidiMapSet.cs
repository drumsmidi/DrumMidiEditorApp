...
public class MidiMapSet : DisposeBaseClass
{
	...
	/// </summary>
	public List<MidiMapGroup> MidiMapGroups { get; private set; } = new();

	/// <summary>
	/// MidiMapGroup位置情報リスト（サブデータ）（描画モード、MidiMapGroupキー、位置情報）
	/// </summary>
	public Dictionary<ConfigPlayer.PlayerSurfaceMode, Dictionary<int,MidiMapGroupPosition>> MidiMapGroupPositions { get; private set; } = new();

	/// <summary>
	/// MidiMapリスト
	/// </summary>
	public List<MidiMap> MidiMaps { get; private set; } = new();

	/// <summary>
	/// 表示対象のMidiMapGroupリスト
	/// </summary>
	public List<MidiMapGroup> DisplayMidiMapGroups { get; private set; } = new();

	/// <summary>
	/// 表示対象のMidiMapリスト
	/// </summary>
	public List<MidiMap> DisplayMidiMaps { get; private set; } = new();

	/// <summary>
	/// 表示対象MidiMapGroup別の表示対象MidiMap数リスト
	/// </summary>
	public List<int> DisplayMidiMapCountByGroup { get; private set; } = new();
	...
	/// </summary>
	public List<int> MidiMapGroupKeys { get; private set; } = new();

	/// <summary>
	/// MidiMapキーリスト（MidiMaps順）
	/// </summary>
	public List<int> MidiMapKeys { get; private set; } = new();

	/// <summary>
	/// MidiMapGroupキーリスト（DisplayMidiMapGroups順）
	/// </summary>
	public List<int> DisplayMidiMapGroupKeys { get; private set; } = new();

	/// <summary>
	/// MidiMapキーリスト（DisplayMidiMaps順）
	/// </summary>
	public List<int> DisplayMidiMapKeys { get; private set; } = new();
	...
	{
		if ( aGroupIndex < 0 || MidiMapGroups.Count < aGroupIndex + 1 )
		{
			return null;
		}
		return MidiMapGroups.ToArray()[ aGroupIndex ];
    }
	...
	{
		if ( aDisplayIndex < 0 || DisplayMidiMapGroups.Count < aDisplayIndex + 1 )
		{
			return null;
		}
		return DisplayMidiMapGroups.ToArray()[ aDisplayIndex ];
	}
    ...
    {
		if ( aMidiMapIndex < 0 || MidiMaps.Count < aMidiMapIndex + 1 )
		{
			return null;
		}
		return MidiMaps.ToArray()[ aMidiMapIndex ];
    }
	...
	{
		if ( aDisplayIndex < 0 || DisplayMidiMaps.Count < aDisplayIndex + 1 )
        {
			return null;
        }
		return DisplayMidiMaps.ToArray()[ aDisplayIndex ];
	}
	...
	public int GetMidiMapGroupKey( int aGroupIndex )
		=> GetMidiMapGroup( aGroupIndex )?.GroupKey ?? Config.System.MidiMapGroupKeyNotSelect ;
	...
	public int GetDisplayMidiMapGroupKey( int aDisplayGroupIndex )
        => GetDisplayMidiMapGroup( aDisplayGroupIndex )?.GroupKey ?? Config.System.MidiMapGroupKeyNotSelect ;
	...
	public int GetMidiMapKey( int aMidiMapIndex )
        => GetMidiMap( aMidiMapIndex )?.MidiMapKey ?? Config.System.MidiMapKeyNotSelect ;
	...
	public int GetDisplayMidiMapKey( int aDisplayMidiMapIndex )
        => GetDisplayMidiMap( aDisplayMidiMapIndex )?.MidiMapKey ?? Config.System.MidiMapKeyNotSelect ;
	...
	{
		int key = Config.System.MidiMapGroupKeyNotSelect;

		for ( int i = 0; i < Config.System.MidiMapGroupMaxNumber; i++ )
		{
	...
	{
		if ( MidiMapGroupKeys.Contains( aNewKey ) )
		{
			return GetMidiMapGroupNewKey();
		}
		return aNewKey;
	}
	...
	{
		int key = Config.System.MidiMapKeyNotSelect;

		for ( int i = 0; i < Config.System.MidiMapMaxNumber; i++ )
		{
	...
	{
		if ( MidiMapKeys.Contains( aNewKey ) )
		{
			return GetMidiMapNewKey();
		}
		return aNewKey;
	}
	...
	public void UpdateInfo()
	{
        ...
        int group_cnt = MidiMapGroups.Count;

		for ( int group_index = 0; group_index < group_cnt; group_index++ )
		{
		...
		foreach ( var group in MidiMapGroups )
        {
			foreach ( ConfigPlayer.PlayerSurfaceMode mode in Enum.GetValues( typeof( ConfigPlayer.PlayerSurfaceMode ) ) )
            {
	...
	{
		int index = GetMidiMapIndex( aMidiMapKey );
	...
	public int GetMidiMapKeyForMatchMidi( byte aMidi )
    {
		...
		return Config.System.MidiMapKeyNotSelect;
    }
