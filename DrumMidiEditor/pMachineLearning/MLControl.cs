...
public static class MLControl
{
	...
	private static IDataView? CreateDataSet( MLContext aMLContext, bool aPrediction, int aStartMeasureNo, int aEndMeasureNo )
    {
		...
		var range = new Rectangle( 0, 0, bgm.Channels, bgm.FFTBufferLength1 );

		for ( int offset_x = 0; offset_x < bgm.FFTBufferLength0 - range.Width; offset_x += range.Width )
        {
			...
			for ( int offset_y = 0; offset_y < bgm.FFTBufferLength1; offset_y += range.Height )
			{ 
				range.Y = offset_y;
				...
				dataSet.Add
					( 
						new()
						{
							...
							HzCenter		= bgm.GetHz( offset_y + range.Height / 2 ),
	...
	private static void BuildAndTrainAndEvaluate( MLContext aMLContext, IDataView aTrainingData )
	{
		...
		IDataView?					trainedModelData	= null;
		...
		// 予測用データの保存
		aMLContext.Model.Save( predictModel, predictModelData.Schema, $"{Config.System.FolderModel.RelativeFolderPath}\\model_predict_{Config.System.OpenFilePath.FileName}.zip" );
	}
	...
	{
		// 保存されたモデルを読み込む
		var loadedModel = aMLContext.Model.Load( $"{Config.System.FolderModel.RelativeFolderPath}\\model_predict_{Config.System.OpenFilePath.FileName}.zip", out var modelInputSchema );
