...
public class DmsItemNoteRange : DisposeBaseClass
{
    ...
    public Rectangle GetSelectRange( Point aStartNotePos )
    {
        if ( !Selected )
        {
            return new( -1, -1, 0, 0 );
        ...
        var r = new Rectangle( _Range.Location, _Range.Size );
        r.X += ( StartNotePosX - aStartNotePos.X ) * _NoteRect.Width;
        r.Y += ( StartNotePosY - aStartNotePos.Y ) * _NoteRect.Height;
    ...
    /// <param name="aStartNotePos">ノート位置（絶対値）</param>
    public void Draw( Graphics aGraphics, Point aStartNotePos )
    {
        if ( !Selected )
        {
            return;
        ...
        var r = GetSelectRange( aStartNotePos );

		aGraphics.DrawRectangle( Config.Editer.NoteRangePen, r );
    }
