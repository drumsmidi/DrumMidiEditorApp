...
public class DmsItemVolumeRange : DisposeBaseClass
{
    ...
    public void SetBottomPosition( int aX, int aY )
    {
        _BottomPoint.X	= aX;
        _BottomPoint.Y	= aY;
    }
    ...
    public void DrawRange( Graphics aGraphics, int aStartNotePosX )
    {
        if ( !_Display )
        {
            return;
        ...
        if ( cnt > 0 )
        {
            int term = Config.Editer.NoteWidthSize * ( _Direct ? 1 : -1 );
            ...
            for ( int i = 0; i < cnt; i++ )
            {
                ...
                aGraphics.DrawLine( Config.Editer.VolumePen, p1, p2 );
