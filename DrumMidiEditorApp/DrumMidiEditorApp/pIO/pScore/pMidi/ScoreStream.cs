...
namespace DrumMidiEditorApp.pIO.pScore.pMidi;
...
internal class ScoreStream : IScoreReader, IScoreWriter
{
    ...
    public void Read( GeneralPath aGeneralPath, out Score aScore )
	{
        ...
        // Truck number
        int truck_cnt = MidiNumber( buffer, pos + 10, 2 );

        // Resolution
        int time_unit = MidiNumber( buffer, pos + 12, 2 );
        int time_unit_cal = time_unit * 4;

        pos += 14;

		float zoom = Config.Media.MidiImportZoom;

		for ( int track = 0; track < truck_cnt; track++ )
		{
			Log.Info( $"---------------------------------" );
            ...
            // Truck size
            int truck_len = MidiNumber( buffer, pos + 4, 4 );
			...
			for ( int i = 0; i < truck_len; i += pos - pos_bef )
			{
				pos_bef = pos;

				// Delta time
                int vardat_len = MidiVariableTextLength( buffer, pos );

                int delta = (int)( MidiVariableNumber( buffer, pos, vardat_len ) * zoom );
                delta_sum	+= delta;
				pos			+= vardat_len;
				...
				switch ( command )
				{
					...
					case 0x80:  // NoteOff
						{
							var midi		= (byte)buffer[ pos ];
							var volume		= (byte)buffer[ pos + 1 ];
							pos			   += 2;

							var measure_no	= delta_sum / time_unit_cal;
							var note_pos	= Config.System.MeasureNoteNumber * ( delta_sum % time_unit_cal ) / time_unit_cal;
							...
							if ( channelItem != null )
                            {
								...
								if ( midiMapKey == Config.System.MidiMapKeyNotSelect )
								{
								...
								case 0x51:   // Tempo setting			[3byte]
                                    {
										...
										var bpm = 0D;
										bpm += (int)buffer[ pos     ] << 16;
										bpm += (int)buffer[ pos + 1 ] <<  8;
										bpm += (int)buffer[ pos + 2 ];
	...
	public void Write( GeneralPath aGeneralPath, Score aScore )
	{
...

		{
			...
			byte midi;
			byte volume;
			int	 bpm = (int)( 120F * 500000F / (float)aScore.Bpm );

			var bpmBytes = BitConverter.GetBytes( bpm );

			data_list.Add( (byte)0x00 );
			data_list.Add( (byte)0xFF );
			data_list.Add( (byte)0x51 );
			data_list.Add( (byte)0x03 );
			...
			for ( int measure_no = 0; measure_no <= measureNoMax; measure_no++ )
			{
				for ( int note_pos = 0; note_pos < Config.System.MeasureNoteNumber; note_pos++ )
				{
					#region bpm
					{
						...
						if ( bpmInfo != null )
						{
							...
							// delta time
							int val = 1920 * count / Config.System.MeasureNoteNumber;
							...
							data_list.Add( (byte)0xFF );	// Meta event
							data_list.Add( (byte)0x51 );    // Tempo setting
							data_list.Add( (byte)0x03 );
							data_list.Add( bpmBytes.Length >= 3 ? bpmBytes[ 2 ] : (byte)0x00 );
							data_list.Add( bpmBytes.Length >= 2 ? bpmBytes[ 1 ] : (byte)0x00 );
							data_list.Add( bpmBytes.Length >= 1 ? bpmBytes[ 0 ] : (byte)0x00 );
					...
					foreach ( var channel in aScore.Channels.Values )
					{
						...
						{ 
							...
							midi		= midiMap.Midi;
							volume		= (byte)Config.Media.CheckMidiVolume( note.Volume + midiMap.VolumeAddIncludeGroup );
							...
							if ( note.NoteOn )
                            {
								if ( note.NoteOff )
								{ 
									data_list.Add( (byte)0x00 );  // Delta time
								}

								data_list.Add( (byte)( 0x90 | channelNo ) );	// NoteOn
								data_list.Add( (byte)midi   );
								data_list.Add( (byte)volume );

								if ( channelNo == Config.Media.ChannelDrum )
                                {
									data_list.Add( (byte)0x00 );  // Delta time
									data_list.Add( (byte)( 0x80 | channelNo ) );	// NoteOff
									data_list.Add( (byte)midi );
		...
		#region Midi Header
		{
			...
			writer.WriteByte( (byte)0x68 );
			writer.WriteByte( (byte)0x64 );
			// data length
			writer.WriteByte( (byte)0x00 );
			writer.WriteByte( (byte)0x00 );
			writer.WriteByte( (byte)0x00 );
			writer.WriteByte( (byte)0x06 );
			// format
			writer.WriteByte( (byte)0x00 );
			writer.WriteByte( (byte)0x00 );
			// truck count
