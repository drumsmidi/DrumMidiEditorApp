...
namespace DrumMidiClassLibrary.pIO.pScore.pMidi;
...
internal class ScoreStream : IScoreReader, IScoreWriter
{
    ...
    public void Read( GeneralPath aGeneralPath, out Score aScore )
	{
		...
		for ( var track = 0; track < truck_cnt; track++ )
		{
			...
			for ( var i = 0; i < truck_len; i += pos - pos_bef )
			{
                ...
                var delta = (int)( MidiVariableNumber( buffer, pos, vardat_len ) * zoom );
                delta_sum	+= delta;
				pos			+= vardat_len;
	...
	public void Write( GeneralPath aGeneralPath, Score aScore )
	{
...

		{
			...
			{
				for ( var note_pos = 0; note_pos < Config.System.MeasureNoteNumber; note_pos++ )
				{
					#region bpm
					{
						...
						if ( bpmInfo != null )
						{
							...
							// delta time
							var val = 1920 * count / Config.System.MeasureNoteNumber;
