...
public class MidiMapSet : DisposeBaseClass
{
    ...
    public MidiMapGroup? GetMidiMapGroup( int aGroupIndex ) => aGroupIndex < 0 || MidiMapGroups.Count < aGroupIndex + 1 ? null : MidiMapGroups.ToArray() [ aGroupIndex ];
    ...
    /// <returns>取得：MidiMapGroup、未取得：null</returns>
    public MidiMapGroup? GetDisplayMidiMapGroup( int aDisplayIndex ) => aDisplayIndex < 0 || DisplayMidiMapGroups.Count < aDisplayIndex + 1 ? null : DisplayMidiMapGroups.ToArray() [ aDisplayIndex ];
    ...
    public MidiMap? GetMidiMap( int aMidiMapIndex ) => aMidiMapIndex < 0 || MidiMaps.Count < aMidiMapIndex + 1 ? null : MidiMaps.ToArray() [ aMidiMapIndex ];
    ...
    /// <returns>取得：MidiMap、未取得：null</returns>
    public MidiMap? GetDisplayMidiMap( int aDisplayIndex ) => aDisplayIndex < 0 || DisplayMidiMaps.Count < aDisplayIndex + 1 ? null : DisplayMidiMaps.ToArray() [ aDisplayIndex ];
    ...
    /// <returns>採番：0-、不採番：-1</returns>
    public int GetMidiMapGroupNewKey( int aNewKey ) => MidiMapGroupKeys.Contains( aNewKey ) ? GetMidiMapGroupNewKey() : aNewKey;
    ...
    /// <param name="aNewKey">キー指定</param>
    /// <returns>採番：0-、不採番：-1</returns>
    public int GetMidiMapNewKey( int aNewKey ) => MidiMapKeys.Contains( aNewKey ) ? GetMidiMapNewKey() : aNewKey;
    ...
    /// </summary>
    public bool IsSelectMidiMap() => DisplayMidiMapGroups.Where( item => item.Selected ).Any() || DisplayMidiMaps.Where( item => item.Selected ).Any();
