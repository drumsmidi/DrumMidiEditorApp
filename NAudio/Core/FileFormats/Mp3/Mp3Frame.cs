...
public class Mp3Frame
{
    ...
    private static bool IsValidHeader( byte [] headerBytes, Mp3Frame frame )
    {
        if ( headerBytes [ 0 ] == 0xFF && ( headerBytes [ 1 ] & 0xE0 ) == 0xE0 )
        {
            ...
            _ = ( headerBytes [ 3 ] & 0x04 ) == 0x04;

            _ = headerBytes [ 3 ] & 0x03;

            var nPadding = padding ? 1 : 0;

            frame.SampleCount = samplesPerFrame [ versionIndex, layerIndex ];
            var coefficient = frame.SampleCount/8;
            frame.FrameLength = frame.MpegLayer == MpegLayer.Layer1
                ? ( ( coefficient * frame.BitRate / frame.SampleRate ) + nPadding ) * 4
                : ( coefficient * frame.BitRate / frame.SampleRate ) + nPadding;
