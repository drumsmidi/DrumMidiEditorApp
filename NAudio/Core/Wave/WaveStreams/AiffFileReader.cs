...
public class AiffFileReader : WaveStream
{
    ...
    public static void ReadAiffHeader( Stream stream, out WaveFormat format, out long dataChunkPosition, out int dataChunkLength, List<AiffChunk> chunks )
    {
        ...
        _ = ConvertInt( br.ReadBytes( 4 ) );
        var formType = ReadChunkName(br);
        if ( formType is not "AIFC" and not "AIFF" )
        {
        ...
        while ( br.BaseStream.Position < br.BaseStream.Length )
        {
            ...
            if ( nextChunk.ChunkName == "COMM" )
            {
                ...
                if ( nextChunk.ChunkLength > 18 && formType == "AIFC" )
                {
                    ...
                    _ = br.ReadBytes( (int)nextChunk.ChunkLength - 22 );
                }
                else
                {
                    _ = br.ReadBytes( (int)nextChunk.ChunkLength - 18 );
                }
            }
    ...
    /// </summary>
    public long SampleCount => waveFormat.Encoding is WaveFormatEncoding.Pcm or
    ...
    private static short ConvertShort( byte [] buffer ) => buffer.Length != 2 ? throw new Exception( "Incorrect length for int." ) : (short)( ( buffer [ 0 ] << 8 ) | buffer [ 1 ] );
    #endregion
    ...
    private static string ReadChunkName( BinaryReader br ) => new( br.ReadChars( 4 ) );
    #endregion
}
