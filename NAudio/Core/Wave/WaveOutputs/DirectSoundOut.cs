...
public class DirectSoundOut : IWavePlayer
{
    ...
    public static IEnumerable<DirectSoundDeviceInfo> Devices
    {
        ...
        {
            devices = [];
            DirectSoundEnumerate( new DSEnumCallback( EnumCallback ), nint.Zero );
            return devices;
        }
    }
    ...
    private static bool EnumCallback( nint lpGuid, nint lpcstrDescription, nint lpcstrModule, nint lpContext )
    {
    ...
    public long GetPosition()
    {
        if ( PlaybackState != PlaybackState.Stopped )
        {
            ...
            if ( sbuf != null )
            {
                ...
                sbuf.GetCurrentPosition( out currentPlayCursor, out _ );
    ...
    public TimeSpan PlaybackPosition
    {
        get
        {
            ...
            pos /= OutputWaveFormat.Channels * OutputWaveFormat.BitsPerSample / 8;

            // ms played in this stream
            return TimeSpan.FromMilliseconds( pos * 1000.0 / OutputWaveFormat.SampleRate );
        }
    }
    ...
    private void InitializeDirectSound()
    {
        ...
        lock ( m_LockObject )
        {
            ...
            DirectSoundCreate( ref device, out directSound, nint.Zero );
            ...
            if ( directSound != null )
            {
                ...
                bufferDesc.lpwfxFormat = nint.Zero;
                bufferDesc.guidAlgo = Guid.Empty;

                object soundBufferObj;
                // Create PrimaryBuffer
                directSound.CreateSoundBuffer( bufferDesc, out soundBufferObj, nint.Zero );
                ...
                var handleOnWaveFormat = GCHandle.Alloc(OutputWaveFormat, GCHandleType.Pinned); // Ptr to waveFormat
                bufferDesc2.lpwfxFormat = handleOnWaveFormat.AddrOfPinnedObject(); // set Ptr to waveFormat
                bufferDesc2.guidAlgo = Guid.Empty;

                // Create SecondaryBuffer
                directSound.CreateSoundBuffer( bufferDesc2, out soundBufferObj, nint.Zero );
                ...
                {
                    dwOffset = 0,
                    hEventNotify = frameEventWaitHandle1.SafeWaitHandle.DangerousGetHandle()
                };

                notifies [ 1 ] = new DirectSoundBufferPositionNotify
                {
                    dwOffset = (uint)samplesFrameSize,
                    hEventNotify = frameEventWaitHandle2.SafeWaitHandle.DangerousGetHandle()
                };

                notifies [ 2 ] = new DirectSoundBufferPositionNotify
                {
                    dwOffset = 0xFFFFFFFF,
                    hEventNotify = endEventWaitHandle.SafeWaitHandle.DangerousGetHandle()
                };
    ...
    public float Volume
    {
        get => 1.0f;//return 1 + (secondaryBuffer.GetVolume()) / 10000.0f;
        set
        {
            if ( value != 1.0f )
            {
    ...
    /// </returns>
    private bool IsBufferLost() => ( secondaryBuffer.GetStatus() & DirectSoundBufferStatus.DSBSTATUS_BUFFERLOST ) != 0;
    ...
    {
        var bytes = ms * (OutputWaveFormat.AverageBytesPerSecond / 1000);
    ...
    private void StopPlayback()
    {
        lock ( m_LockObject )
        {
            if ( secondaryBuffer != null )
            {
                ...
                _ = Marshal.ReleaseComObject( secondaryBuffer );
                secondaryBuffer = null;
            }
            if ( primarySoundBuffer != null )
            {
                primarySoundBuffer.Stop();
                _ = Marshal.ReleaseComObject( primarySoundBuffer );
                primarySoundBuffer = null;
            }
            if ( directSound != null )
            {
                _ = Marshal.ReleaseComObject( directSound );
                directSound = null;
            }
        }
    }
    ...
    private void CleanUpSecondaryBuffer()
    {
        if ( secondaryBuffer != null )
        {
            ...
            if ( wavBuffer1 != nint.Zero )
            {
                Marshal.Copy( silence, 0, wavBuffer1, nbSamples1 );
                if ( wavBuffer2 != nint.Zero )
                {
                    Marshal.Copy( silence, 0, wavBuffer1, nbSamples1 );
                }
            }
    ...
    internal interface IDirectSound
    {
        ...
        void CreateSoundBuffer( [In] BufferDescription desc, [Out, MarshalAs( UnmanagedType.Interface )] out object dsDSoundBuffer, nint pUnkOuter );
        //STDMETHOD(GetCaps)              (THIS_ LPDSCAPS pDSCaps) PURE;
        void GetCaps( nint caps );
        ...
        void SetCooperativeLevel( nint HWND, [In, MarshalAs( UnmanagedType.U4 )] DirectSoundCooperativeLevel dwLevel );
        //STDMETHOD(Compact)              (THIS) PURE;
        void Compact();
        //STDMETHOD(GetSpeakerConfig)     (THIS_ LPDWORD pdwSpeakerConfig) PURE;
        void GetSpeakerConfig( nint pdwSpeakerConfig );
    ...
    internal interface IDirectSoundBuffer
    {
        ...
        void Lock( int dwOffset, uint dwBytes, [Out] out nint audioPtr1, [Out] out int audioBytes1, [Out] out nint audioPtr2, [Out] out int audioBytes2, [MarshalAs( UnmanagedType.U4 )] DirectSoundBufferLockFlag dwFlags );
        ...
        void Unlock( nint pvAudioPtr1, int dwAudioBytes1, nint pvAudioPtr2, int dwAudioBytes2 );
    ...
    private static extern void DirectSoundCreate( ref Guid GUID, [Out, MarshalAs( UnmanagedType.Interface )] out IDirectSound directSound, nint pUnkOuter );
    ...
    private delegate bool DSEnumCallback( nint lpGuid, nint lpcstrDescription, nint lpcstrModule, nint lpContext );
    ...
    private static extern void DirectSoundEnumerate( DSEnumCallback lpDSEnumCallback, nint lpContext );
