...
public class WaveInProvider : IWaveProvider
{
    ...
    private void OnDataAvailable( object sender, WaveInEventArgs e ) => bufferedWaveProvider.AddSamples( e.Buffer, 0, e.BytesRecorded );

    /// <summary>
    /// Reads data from the WaveInProvider
    /// </summary>
    public int Read( byte [] buffer, int offset, int count ) => bufferedWaveProvider.Read( buffer, offset, count );
