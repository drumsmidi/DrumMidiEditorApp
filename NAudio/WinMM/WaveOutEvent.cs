...
public class WaveOutEvent : IWavePlayer, IWavePosition
{
    ...
    public void Play()
    {
        ...
        if ( playbackState == PlaybackState.Stopped )
        {
            ...
            _ = callbackEvent.Set(); // give the thread a kick
            _ = ThreadPool.QueueUserWorkItem( state => PlaybackThread(), null );
        }
    ...
    public void Stop()
    {
        if ( playbackState != PlaybackState.Stopped )
        {
            ...
            _ = callbackEvent.Set(); // give the thread a kick, make sure we exit
        }
    }
    ...
    private void CloseWaveOut()
    {
        ...
        lock ( waveOutLock )
        {
            if ( hWaveOut != nint.Zero )
            {
                _ = WaveInterop.waveOutClose( hWaveOut );
                hWaveOut = nint.Zero;
            }
        }
    }
