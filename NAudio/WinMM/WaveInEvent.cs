...
public class WaveInEvent : IWaveIn
{
    ...
    {
        CloseWaveInDevice();
        var result = WaveInterop.waveInOpenWindow(out waveInHandle, DeviceNumber, WaveFormat,
            callbackEvent.SafeWaitHandle.DangerousGetHandle(),
            nint.Zero, WaveInterop.WaveInOutOpenFlags.CallbackEvent);
    ...
    public void StartRecording()
    {
        ...
        _ = ThreadPool.QueueUserWorkItem( ( state ) => RecordThread(), null );
    }
    ...
    public void StopRecording()
    {
        if ( captureState != CaptureState.Stopped )
        {
            ...
            _ = callbackEvent.Set(); // signal the thread to exit
        }
    }
    ...
    private void CloseWaveInDevice()
    {
        ...
        _ = WaveInterop.waveInReset( waveInHandle );
        if ( buffers != null )
        {
            for ( var n = 0; n < buffers.Length; n++ )
            {
        ...
        _ = WaveInterop.waveInClose( waveInHandle );
    ...
    public MixerLine GetMixerLine()
    {
        ...
        var mixerLine = waveInHandle != nint.Zero
            ? new MixerLine( waveInHandle, 0, MixerFlags.WaveInHandle )
            : new MixerLine( DeviceNumber, 0, MixerFlags.WaveIn );
        return mixerLine;
    }
