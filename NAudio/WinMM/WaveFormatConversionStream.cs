...
public class WaveFormatConversionStream : WaveStream
{
    ...
    public static WaveStream CreatePcmStream( WaveStream sourceStream )
    {
        ...
        {
            pcmFormat = sourceStream.WaveFormat.Encoding == WaveFormatEncoding.G723
    ...
    public int SourceToDest( int source ) => (int)EstimateSourceToDest( source );//return conversionStream.SourceToDest(source);
    ...
    /// </summary>
    [Obsolete( "can be unreliable, use of this method not encouraged" )]
    public int DestToSource( int dest ) => (int)EstimateDestToSource( dest );//return conversionStream.DestToSource(dest);
